...
using CromulentBisgetti.ContainerPacking.Algorithms.EBAFIT;
using CromulentBisgetti.ContainerPacking.Entities;
using CromulentBisgetti.ContainerPacking.Utils;
using System;
...
namespace CromulentBisgetti.ContainerPacking
{
    ...
    public static class PackingService
    {
        ...
        public static List<ContainerPackingResult> Pack(List<Container> containers, List<Item> itemsToPack, List<int> algorithmTypeIDs)
        {
            ...
            List<ContainerPackingResult> result = new List<ContainerPackingResult>(containers.Count * algorithmTypeIDs.Count);
            ...
            Parallel.ForEach(containers, container =>
            {
                ...
                Parallel.ForEach(algorithmTypeIDs, algorithmTypeID =>
                {
                    ...
                    List<Item> items = new List<Item>(itemsToPack.Count);
                    for (var i = 0; i < itemsToPack.Count; i++)
                    {
                        var item = itemsToPack[i];
                ...
                containerPackingResult.AlgorithmPackingResults.Sort(AlgorithmPackingResultNameComparison.Comparer);
        ...
        {
            if (algorithmTypeID == (int)AlgorithmType.EB_AFIT) return new EB_AFIT();
